...
const CampaignBuilder: React.FC = () => {
  ...
  const handleSaveCampaign = async () => {
    ...
    if (rules.length === 0) {
      setError('Please generate targeting rules first');
      return;
    }
    ...
    try {
      const newCampaign = await campaignApi.create({
        name: campaignName,
        audienceQuery: rules,
        summary: messageSuggestion || undefined,
        audienceSize: Math.floor(Math.random() * 500) + 100, // Mock data for now
        sentCount: 0,
        failedCount: 0,
        deliveryRatio: 0,
        status: 'in-progress'
      ...
      setError('Failed to save campaign to database');
    } finally {
            ...
            <div className="bg-white p-8 rounded-xl shadow-lg border border-indigo-100 h-full">
              ...
              {rules.length > 0 ? (
                <div className="space-y-4">
                  <ul className="space-y-3">
                    {rules.map((rule, index) => (
                      <li key={index} className="p-4 bg-green-50 border border-green-200 rounded-lg flex items-start">
                        ...
                        <div>
                          ...
                          <div className="text-gray-800">
                            ...
                            <span className="mx-1">{rule.operator}</span>
                  ...
                  <button
                    ...
                    className="w-full py-3 px-6 mt-6 bg-green-600 hover:bg-green-700 text-white font-medium rounded-lg shadow-md hover:shadow-lg transition-all focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50 flex items-center justify-center"
                    disabled={savingCampaign || !campaignName || rules.length === 0}
                  >
                    {savingCampaign ? (
                      <span className="flex items-center">
                        <svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                ...
                <div className="h-64 flex flex-col items-center justify-center text-gray-400">
                  ...
                  <p className="text-center">Enter a description and click "Generate Rules" to create targeting rules</p>
                </div>
              )}
            </div>
          </div>
